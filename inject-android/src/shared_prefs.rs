        ...
        for entry in &self.entries {
            match &entry.value {
                ...
                SharedPrefsValue::IntVal(v) => {
                    xml.push_str(&format!("    <int name=\"{}\" value=\"{v}\" />\n", entry.key));
                }
                SharedPrefsValue::LongVal(v) => {
                    xml.push_str(&format!("    <long name=\"{}\" value=\"{v}\" />\n", entry.key));
                }
                SharedPrefsValue::FloatVal(v) => {
                    xml.push_str(&format!("    <float name=\"{}\" value=\"{v}\" />\n", entry.key));
                }
                SharedPrefsValue::BoolVal(v) => {
                    xml.push_str(&format!("    <boolean name=\"{}\" value=\"{v}\" />\n", entry.key));
                }
            }
        }
        xml.push_str("</map>\n");
...
mod tests {
    ...
    fn sample_records() -> Vec<SharedPrefsRecord> {
        vec![SharedPrefsRecord {
            ...
            entries: vec![
                SharedPrefsEntry { key: "username".into(), value: SharedPrefsValue::StringVal("demo_user".into()) },
                SharedPrefsEntry { key: "login_count".into(), value: SharedPrefsValue::IntVal(42) },
                SharedPrefsEntry { key: "dark_mode".into(), value: SharedPrefsValue::BoolVal(true) },
            ],
        }]
    }
    ...
    fn test_inject_and_verify_shared_prefs() {
        ...
        let content = std::fs::read_to_string(dir.path().join("com.example.app_user_settings.xml")).unwrap();
