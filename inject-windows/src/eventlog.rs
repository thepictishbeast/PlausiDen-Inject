...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum EventLevel { Information, Warning, Error, Critical, Audit }

impl EventLogRecord {
    /// Render as EVTX XML format.
    pub fn to_xml(&self) -> String {
        format!(r#"<Event xmlns="http://schemas.microsoft.com/win/2004/08/events/event">
  <System>
  ...
  <EventData><Data>{message}</Data></EventData>
            ...
            source = self.source, event_id = self.event_id,
            level = match self.level { EventLevel::Information => 4, EventLevel::Warning => 3, EventLevel::Error => 2, EventLevel::Critical => 1, EventLevel::Audit => 0 },
            timestamp = self.timestamp.to_rfc3339(), channel = self.channel,
            computer = self.computer, message = self.message,
        )
    }
}

pub struct EventLogInjector { output_dir: PathBuf }
impl EventLogInjector { pub fn new(dir: PathBuf) -> Self { Self { output_dir: dir } } }
...
mod tests {
    ...
    fn test_eventlog_xml_format() {
        let record = EventLogRecord { event_id: 4624, level: EventLevel::Information, source: "Microsoft-Windows-Security-Auditing".into(), channel: "Security".into(), computer: "WORKSTATION".into(), timestamp: Utc::now(), message: "An account was successfully logged on.".into() };
    ...
    fn test_inject_eventlog() {
        ...
        let records = vec![
            EventLogRecord { event_id: 4624, level: EventLevel::Information, source: "Security".into(), channel: "Security".into(), computer: "PC".into(), timestamp: Utc::now(), message: "Logon success".into() },
            EventLogRecord { event_id: 7045, level: EventLevel::Information, source: "Service Control Manager".into(), channel: "System".into(), computer: "PC".into(), timestamp: Utc::now(), message: "Service installed".into() },
        ];
