...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum ShowCommand { Normal, Minimized, Maximized }

pub struct LnkInjector { output_dir: PathBuf }
...
impl Injector for LnkInjector {
    fn inject(&self, artifact_bytes: &[u8], _target: &Target, _strategy: InjectionStrategy) -> Result<InjectionResult> {
        ...
        for record in &records {
            let name = std::path::Path::new(&record.target_path).file_stem()
                .and_then(|s| s.to_str()).unwrap_or("shortcut");
    ...
    fn verify(&self, result: &InjectionResult) -> Result<VerificationStatus> {
        let present = result.injected_ids.iter().filter(|p| std::path::Path::new(p).exists()).count();
        if present == result.injected_ids.len() { Ok(VerificationStatus::AllPresent { checked: present }) }
        else if present > 0 { Ok(VerificationStatus::PartiallyPresent { present, missing: result.injected_ids.len() - present, missing_ids: vec![] }) }
        else { Ok(VerificationStatus::NonePresent { expected: result.injected_ids.len() }) }
    }

    fn rollback(&self, result: &InjectionResult) -> Result<()> {
        for p in &result.injected_ids { let _ = std::fs::remove_file(p); }
        Ok(())
    }
...
mod tests {
    ...
    fn test_inject_lnk() {
        ...
        let records = vec![LnkRecord {
            target_path: "C:\\Program Files\\app.exe".into(), working_dir: "C:\\Program Files".into(),
            description: "Application".into(), icon_location: None,
            created_at: Utc::now(), modified_at: Utc::now(), accessed_at: Utc::now(),
            target_size: 1024, show_command: ShowCommand::Normal,
        }];
    ...
    fn test_verify_and_rollback() {
        ...
        let injector = LnkInjector::new(dir.path().into());
        let records = vec![LnkRecord {
            target_path: "C:\\test.txt".into(), working_dir: "C:\\".into(),
            description: "Test".into(), icon_location: None,
            created_at: Utc::now(), modified_at: Utc::now(), accessed_at: Utc::now(),
            target_size: 100, show_command: ShowCommand::Normal,
        }];
        let bytes = serde_json::to_vec(&records).unwrap();
        let result = injector.inject(&bytes, &Target::Filesystem { path: dir.path().into() }, InjectionStrategy::DirectInjection).unwrap();
